        ...
        return await withCheckedContinuation { continuation in
            ...
            let executeOperation = ClosureOperation { [weak self] in
                Task { [weak self] in
                    ...
                    let accountInfoOperations: [ClosureOperation<[ChainAsset: AccountInfo?]>] = await rawResult
...
private extension AccountInfoFetching {
    ...
    private func createDecodeOperation(
        ...
        case .normal:
            guard let runtimeCodingService = await chainRegistry.getRuntimeProvider(for: chainAsset.chain.chainId) else {
                return ClosureOperation { [chainAsset: nil] }
            }
