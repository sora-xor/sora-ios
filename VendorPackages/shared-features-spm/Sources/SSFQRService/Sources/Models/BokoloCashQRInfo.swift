...
final class BokoloCashDecoder: QRDecoder {
    func decode(data: Data) throws -> QRInfoType {
        guard let incomingURL = URL(dataRepresentation: data, relativeTo: nil, isAbsolute: true),
              let payload = fetchPayloadData(url: incomingURL) else
        {
