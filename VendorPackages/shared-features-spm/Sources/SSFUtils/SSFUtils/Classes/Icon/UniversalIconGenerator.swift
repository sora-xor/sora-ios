...
public class UniversalIconGenerator: IconGenerating {
    ...
    private func deriveAccountIdFromAddress(_ address: String) throws -> Data {
        ...
        switch chainFormat {
        case .sfEthereum:
            accountId = try AccountId(hexStringSSF: address)
        case let .sfSubstrate(prefix):
            accountId = try SS58AddressFactory().accountId(fromAddress: address, type: prefix)
        }
        ...
        for (index, byte) in accountId.enumerated() {
            ...
            if colorParam == 0 {
                ...
                let lightness: CGFloat = [
                    ...
                    35.0,
                    75.0,
                ][Int(floor(CGFloat(colorParam) / 64.0))]
