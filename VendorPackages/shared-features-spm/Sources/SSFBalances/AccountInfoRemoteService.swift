...
public actor AccountInfoRemoteServiceDefault: AccountInfoRemoteService {
    ...
    private func fetchSubstrate(
        ...
        let requests = chain.chainAssets
            .map { createSubstrateRequest(for: $0, accountId: accountId) }
    ...
    private func mapAccountInfo(
        response: MixStorageResponse,
        chain: ChainModel
    ) throws -> AccountInfo? {
        ...
        guard let registry = AccountInfoStorageResponseValueRegistry(
            rawValue: response.request
        ...
        case .equilibrium:
            ...
            let chainAssetId = ChainAssetId(
                ...
                assetId: response.request.requestId
            ) // ?? ChainAssetId(id: response.request.requestId)
            guard let chainAsset = chain.chainAssets
                .first(where: { $0.chainAssetId == chainAssetId }),
                let currencyId = chainAsset.asset.tokenProperties?.currencyId else
            {
                return nil
            }
    ...
    private func createSubstrateRequest(
        for chainAsset: ChainAsset,
        accountId: AccountId
        ...
        case .none:
            let parametersType: MixStorageRequestParametersType
            if chainAsset.chain.chainId == Chain.reef.genesisHash || chainAsset.chain
                .chainId == Chain.scuba.genesisHash
            {
