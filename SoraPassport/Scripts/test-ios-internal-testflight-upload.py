...
INTERNAL_TAIRA_CONFIG = (
    ROOT / "Fixtures" / "Modernization" / "ios-taira-internal-testflight-v1.json"
)
    ...
    environment.update(
        {
            ...
            "SORA_IOS_INTERNAL_TESTFLIGHT_BUILD_NUMBER": "2026082006",
            ...
            "CURRENT_PROJECT_VERSION": "2026082006",
            "SWIFT_ACTIVE_COMPILATION_CONDITIONS":
                "F_RELEASE SORA_INTERNAL_TAIRA_TESTFLIGHT",
            "OTHER_SWIFT_FLAGS": "-DF_RELEASE",
            "SORA_TAIRA_INTERNAL_TESTFLIGHT_BUILD_NUMBER": "2026082006",
            ...
            "SORA_TAIRA_INTERNAL_TESTFLIGHT_CONTRACT_ID":
                "sora-ios-taira-internal-testflight-v1",
            ...
            "SORA_TAIRA_INTERNAL_TESTFLIGHT_SOURCE_REVISION": revision,
...
class InternalTestFlightUploadTests(unittest.TestCase):
    ...
    def test_wrapper_binds_source_and_stays_non_authorizing(self) -> None:
        ...
        for marker in (
            ...
            "rev-parse '@{upstream}'",
            'reviewed_base_revision="956270b8b3efc5a3b11c68a931eeb877209184ab"',
            'reviewed_upstream="origin/codex/taira-network-switch-20260819"',
            'reviewed_build_number="2026082006"',
            'reviewed_lower_bound="2026082005"',
        ...
        for marker in (
            ...
            "956270b8b3efc5a3b11c68a931eeb877209184ab",
            ...
            "Distributions": [
                {
                    ...
                    "uploadedBuildNumber": "2026082006",
...
""".format(profile_path=reviewed_profile)
    ...
    def test_wrapper_rejects_unreviewed_build_before_xcode(self) -> None:
        ...
        for stale_value in ("2026082004", "2026082003"):
            with self.subTest(stale_lower_bound=stale_value):
                ...
                self.assertNotEqual(stale_lower_bound.returncode, 0)
                self.assertIn(
                    "App Store build-number lower bound is not the reviewed observation",
                    stale_lower_bound.stderr,
                )
                self.assertNotIn("xcodebuild", stale_lower_bound.stderr)
    ...
    def test_action_without_capability_fails_closed(self) -> None:
        ...
        for key, value in (
            ("SWIFT_ACTIVE_COMPILATION_CONDITIONS", "SORA_INTERNAL_TAIRA_TESTFLIGHT"),
            ("SORA_TAIRA_INTERNAL_TESTFLIGHT_CONTRACT_ID", "unexpected"),
        ):
            with self.subTest(escaped=key):
                escaped = dict(os.environ)
                escaped.pop("SORA_IOS_INTERNAL_TESTFLIGHT_UPLOAD_MODE", None)
                escaped.pop("SORA_IOS_INTERNAL_TESTFLIGHT_UPLOAD_ACTION", None)
                escaped[key] = value
                rejected = run("/bin/sh", str(VERIFIER), environment=escaped)
                self.assertNotEqual(rejected.returncode, 0)
                self.assertIn("escaped the internal TestFlight uploader", rejected.stderr)
