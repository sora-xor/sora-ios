...
class MigrationReleaseBoundaryTests(unittest.TestCase):
    ...
    def test_release_build_exception_is_exact_and_observed_only(self) -> None:
        ...
        policy_model = (
            ROOT / "SoraPassport/Common/Model/WalletNetworkModel.swift"
        ).read_text(encoding="utf-8")
        policy_root = (
            ROOT / "SoraPassport/ModulesRedesign/Root/RootInteractor.swift"
        ).read_text(encoding="utf-8")
        policy_cases = (
            ("current", policy_model, policy_root, True),
            ("legacy-derivation", policy_model.replace(
                "self == .mnemonicEntropy\n",
                "self == .mnemonicEntropy || self == .legacyMnemonicEntropy\n", 1
            ), policy_root, False),
            ("retained-direct-derivation", policy_model.replace(
                "WalletMnemonicWordPolicy.isUserImportWordCount(words.count)",
                "WalletMnemonicWordPolicy.retainedSoraWordCounts.contains(words.count)", 1
            ), policy_root, False),
            ("legacy-activation-children", policy_model, policy_root.replace(
                "expectedSource == .legacyMnemonicEntropy\n                ? [.sora2]\n"
                "                : NexusNetworkConfiguration.admittedWalletNetworkIds",
                "NexusNetworkConfiguration.admittedWalletNetworkIds", 1
            ), False),
        )
        with tempfile.TemporaryDirectory(prefix="retained-mnemonic-policy-") as temporary:
            model_fixture = Path(temporary) / "WalletNetworkModel.swift"
            root_fixture = Path(temporary) / "RootInteractor.swift"
            for case, model_source, root_source, accepted in policy_cases:
                with self.subTest(retained_mnemonic_policy=case):
                    model_fixture.write_text(model_source, encoding="utf-8")
                    root_fixture.write_text(root_source, encoding="utf-8")
                    checked = run(
                        "/bin/sh", str(DEPENDENCIES),
                        "--lint-ios-retained-mnemonic-policy",
                        str(model_fixture), str(root_fixture),
                    )
                    self.assertEqual(checked.returncode == 0, accepted, checked.stderr)
        self.assertIn(
