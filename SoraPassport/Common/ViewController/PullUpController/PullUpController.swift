...
// swiftlint:disable line_length file_length identifier_name
...
open class PullUpController: UIViewController {
    ...
    open func pullUpControllerMoveToVisiblePoint(_ visiblePoint: CGFloat, animated: Bool, completion: (() -> Void)?) {
        guard isPortrait, let parentViewHeight = parent?.view.frame.height else { return }
    ...
    fileprivate func setup(superview: UIView, offset: CGFloat) {
        self.initialStickyPointOffset = offset
        view.translatesAutoresizingMaskIntoConstraints = false
        superview.addSubview(view)
        view.frame = CGRect(
            origin: CGPoint(x: view.frame.origin.x,
        ...
        refreshConstraints(
            ...
            customTopOffset: superview.frame.height - offset
        )
    }
    ...
    private func setupConstraints() {
        ...
        let constraintsToActivate = [
            topConstraint, leftConstraint, widthConstraint, heightConstraint, bottomConstraint
        ].compactMap { $0 }
    ...
    private func setTopOffset(_ value: CGFloat,
        ...
        guard let parentViewHeight = parent?.view.frame.height else { return }
    ...
    private func setPortraitConstraints(parentViewSize: CGSize, customTopOffset: CGFloat? = nil) {
        ...
        leftConstraint?.constant = 0//(parentViewSize.width - min(pullUpControllerPreferredSize.width, parentViewSize.width))/2
...
extension UIViewController {
    ...
    /**
     ...
     - parameter initialOffset: The point where the provided `pullUpController`'s view will be initially placed expressed in screen units of the pull up controller coordinate system. If this value is not provided, the `pullUpController`'s view will be initially placed expressed
    ...
    open func addPullUpController(_ pullUpController: PullUpController,
        ...
        pullUpController.setup(superview: view, offset: initialOffset ?? view.frame.height)
...
// swiftlint:enable type_body_length file_length line_length identifier_name
