...
final class IrohaConnectWalletProvider {
    ...
    func context(for launch: IrohaConnectLaunch) throws -> IrohaConnectWalletContext {
        ...
        guard
            ...
            configuration.isCurrentIrohaConnectConfiguration
        else {
            throw IrohaConnectError.walletUnavailable
        }
        do {
...
private extension NexusNetworkConfiguration {
    var isCurrentIrohaConnectConfiguration: Bool {
        switch networkId {
        case .taira:
            guard let deployment = TairaDeploymentBinding.admittedFromBundle else {
                return false
            }
            return chainId == deployment.currentChainId &&
                toriiURL == deployment.canonicalToriiBaseURL &&
                explorerURL.absoluteString == "https://taira-explorer.sora.org" &&
