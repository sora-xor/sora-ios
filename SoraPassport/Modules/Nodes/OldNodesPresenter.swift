...
final class OldNodesPresenter {
    weak var view: OldNodesViewProtocol?
...
extension OldNodesPresenter: NodesPresenterProtocol, AuthorizationPresentable {
...
extension OldNodesPresenter: NodesInteractorOutputProtocol {
    func restart() {
...
extension OldNodesPresenter: NodesCellDelegate {
...
private extension OldNodesPresenter {
    func createItems(with nodes: [NodeViewModel], customNodes: [NodeViewModel]) -> [CellViewModel] {
        var items: [CellViewModel] = [SpaceViewModel(height: 16, backgroundColor: R.color.baseBackground()!)]

        items.append(NodesHeaderViewModel(title: R.string.localizable.selectNodeDefaultNodes(preferredLanguages: .currentLocale)))
        items.append(SpaceViewModel(height: 2, backgroundColor: R.color.baseBackground()!))
        items.append(OldNodesViewModel(nodesModels: nodes, delegate: self))
        items.append(SpaceViewModel(height: 16, backgroundColor: R.color.baseBackground()!))

        if !customNodes.isEmpty {
            items.append(NodesHeaderViewModel(title: R.string.localizable.selectNodeCustomNodes(preferredLanguages: .currentLocale)))
            items.append(SpaceViewModel(height: 2, backgroundColor: R.color.baseBackground()!))
            items.append(OldNodesViewModel(nodesModels: customNodes, delegate: self))
            items.append(SpaceViewModel(height: 16, backgroundColor: R.color.baseBackground()!))
        }

        items.append(ButtonViewModel(title: R.string.localizable.selectNodeAddCustomNode(preferredLanguages: .currentLocale),
                                     titleColor: R.color.brandSoramitsuRed(),
                                     backgroundColor: R.color.neumorphism.backgroundLightGrey(),
                                     delegate: self))
...
extension OldNodesPresenter: ButtonCellDelegate {
    func buttonTapped() {
        onAction(.add)
    }
}
...
extension OldNodesPresenter: Localizable {
