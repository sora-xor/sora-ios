...
- Replacing an existing durable record first creates a random exclusive
  ...
  inside the existing 223-method source suite.
- A positive `retainedReleaseSnapshotCount`, the reviewed
  ...
  `testResultBundleSha256` covering all four suite inventories (223 + 11 + 12 + 3 = 249).
...
scheme. Its exact test inventory is 223 `WalletModernizationTests`, 11
...
uses 19 `WalletUXTests` instead of the three retained-device tests, for 265 tests
(223 + 11 + 12 + 19). The regressions also cover canonical interrupted and failed startup journals, marker-preserving retry, latest privacy-safe diagnostics, and mixed framework and wallet Keychain
...
- `tests/Migration.xcresult`, containing the exact 249 passing test identifiers and three
...
declared suite counts (223 + 11 + 12 + 3 = 249) and zero failure, unexpected-failure, skipped, and
